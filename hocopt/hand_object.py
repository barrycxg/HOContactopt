...
from hocopt.diffcontact import calculate_contact_capsule, calculate_penetration_cost
...
class HandObject:
    ...
    def load_from_image(self, hand_beta, hand_pose, obj_faces, obj_verts, hand_verts=None):
        ...
        if hand_verts is not None:
            ...
            if mean_err.mean() > 1e-2:  # Check if there's much error in reconstruction
    ...
    def load_from_batch(self, hand_beta, hand_pose, hand_mTc, hand_contact, obj_contact, obj_mesh, idx=0, obj_rot=None):
        ...
        if obj_rot is not None: # centroid unchanged rot
    ...
    def run_mano(self):
        ...
        if self.hand_pose.shape[0] == 48:   # Special case when we're loading GT honnotate
            mano_model = ManoLayer(mano_root='mano/models', joint_rot_mode="axisang", use_pca=True, ncomps=45, center_idx=None, flat_hand_mean=False) # dexycb hand gt mano model
            # mano_model = ManoLayer(mano_root='mano/models', joint_rot_mode="axisang", use_pca=False, ncomps=45, center_idx=None, flat_hand_mean=True) # ho3d hand gt mano model
            # if test: # for dexycb and ho3d datasets
            #     mano_model = ManoLayer(mano_root='mano/models', joint_rot_mode="axisang", use_pca=True, ncomps=45, center_idx=None, flat_hand_mean=False)
            # else: # for contactpose dataset
    ...
    def get_o3d_meshes(self, hand_contact=False, obj_contact=False, normalize_pos=False):
        ...
        # hand_color = np.asarray([255.0, 255.0, 255.0]) / 255
        # obj_color = np.asarray([100.0, 100.0, 100.0]) / 255

        hand_color = [0.85882353, 0.74117647, 0.65098039]
        obj_color = [145/255, 191/255, 219/255]
        ...
        if obj_contact and self.obj_contact.mean() != 0:
    ...
    def calc_penetration_depth(self):
        ...
        object_mesh = Meshes(verts=[torch.Tensor(self.obj_verts)], faces=[torch.Tensor(self.obj_faces)])
        obj_verts = object_mesh.verts_padded()
        obj_normals = object_mesh.verts_normals_padded()
        is_thin = util.mesh_is_thin(torch.Tensor([obj_verts.size()[1]]))

        pene_depth = calculate_penetration_cost(hand_verts, hand_normals, obj_verts, obj_normals, is_thin, contact_norm_method=6, allowable_pen=0)
        # pene_depth = pene_depth.sum(dim=1)/(pene_depth[0][pene_depth[0]>0].size()[0] + 0.000001)
        max_pene_depth, _ = pene_depth.max(dim=1)

        # return pene_depth.detach().cpu().numpy()
        return max_pene_depth.detach().cpu().numpy()
